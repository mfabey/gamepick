 ...
 */
export async function appendMessage({ from, to, text, media }) {
  ...
  // `media` yalnızca VARSA ekleniyor: eski mesajlarda alan hiç yok ve
  // `media: undefined` yazmak JSON.stringify'da zaten düşerdi — okurken
  // "alan yok" ile "alan boş" ayrımı yapmak zorunda kalmayalım.
  const msg = media ? { id, from, text, at, media } : { id, from, text, at };
  ...
  await redisPipeline([
    ...
    ['SET', metaKey(cid), JSON.stringify({
      ...
      lastKind: media ? (media.type?.startsWith('video/') ? 'video' : 'photo') : null,
