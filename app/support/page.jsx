...
export default function SupportPage() {
  ...
  const reset = () => { setSent(false); setName(''); setEmail(''); setMsg(''); setSubject('Genel Soru'); };
    ...
    <div className="page-transition" style={{ minHeight: '100vh', background: 'var(--bg-body)', paddingBottom: 100 }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '62px 0 44px', background: 'var(--hero-bg)' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '0 36px', textAlign: 'center' }}>
          <p style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 14 }}>Yardım Merkezi</p>
          <h1 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 'clamp(34px,4.4vw,52px)', lineHeight: 1.06, letterSpacing: '-1.4px', color: 'var(--text)', marginBottom: 16, textWrap: 'balance' }}>Size nasıl yardımcı olabiliriz?</h1>
          <p style={{ fontSize: 17, color: 'var(--text-2)', maxWidth: 520, margin: '0 auto 30px', lineHeight: 1.55 }}>Bir konu seç, sık sorulanlara göz at ya da doğrudan bize yaz. Genelde 24 saat içinde dönüyoruz.</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg-card)', border: '1.5px solid var(--border-hover)', borderRadius: 14, height: 58, padding: '0 18px', maxWidth: 540, margin: '0 auto', boxShadow: 'var(--shadow-lg)' }}>
            ...
            <input placeholder="Bir soru veya konu ara… (ör. fiyat alarmı)" style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'inherit', fontSize: 15.5, color: 'var(--text)' }} />
          </div>
        </div>
      </section>
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '46px 36px 0' }}>
        ...
        {/* ── Konu kartları ── */}
        <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 22, letterSpacing: '-0.5px', color: 'var(--text)', marginBottom: 18 }}>Konuya göz at</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 18, marginBottom: 52 }}>
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 15, padding: 20 }}>
            ...
            <div><div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 3 }}>E-posta</div><div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>destek@gamepick.com</div><div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 2 }}>~24 saat içinde yanıt</div></div>
          </div>
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 15, padding: 20 }}>
            ...
            <div><div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 3, display: 'flex', alignItems: 'center', gap: 6 }}>Canlı Destek <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--green)', boxShadow: '0 0 0 3px rgba(47,158,107,0.2)' }} /></div><div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>Şu an çevrimiçi</div><div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 2 }}>Hafta içi 09:00 – 18:00</div></div>
          </div>
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 15, padding: 20 }}>
            ...
            <div><div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 3 }}>Topluluk</div><div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>Discord sunucusu</div><div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 2 }}>Binlerce oyuncuyla anında</div></div>
          </div>
        </div>
        ...
        <div className="support-split" style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 34, alignItems: 'start' }}>
          ...
          <div className="card" style={{ borderRadius: 20, padding: 30, boxShadow: 'var(--shadow)' }}>
            ...
            <p style={{ fontSize: 14.5, color: 'var(--text-2)', marginBottom: 24 }}>Formu doldur, ekibimiz en kısa sürede dönsün.</p>
            ...
            {sent && (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, background: 'var(--green-bg)', border: '1px solid var(--green-border)', borderRadius: 13, padding: '16px 18px', marginBottom: 22 }}>
                ...
                <div><div style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--green)', marginBottom: 2 }}>Mesajın bize ulaştı!</div><div style={{ fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.5 }}>En kısa sürede e-posta ile döneceğiz. <span onClick={reset} style={{ color: 'var(--accent)', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 2 }}>Yeni mesaj gönder</span></div></div>
              </div>
            )}
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 17 }}>
              <div className="support-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-2)', marginBottom: 7 }}>E-posta</label>
                  <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="ornek@posta.com" style={inputBase}
                    onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => e.target.style.borderColor = 'var(--border-hover)'} />
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-2)', marginBottom: 7 }}>Konu</label>
                <div style={{ position: 'relative' }}>
                  <select value={subject} onChange={e => setSubject(e.target.value)} style={{ ...inputBase, padding: '0 40px 0 15px', appearance: 'none', WebkitAppearance: 'none', cursor: 'pointer' }}
                    ...
                    <option>Hesap & Üyelik</option>
                    <option>Fiyat & Mağaza</option>
                    <option>Premium & Ödeme</option>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-2)', marginBottom: 7 }}>Mesajınız</label>
                <textarea value={msg} onChange={e => setMsg(e.target.value)} placeholder="Sorununu veya sorunu olabildiğince ayrıntılı anlat…" style={{ ...inputBase, height: 'auto', minHeight: 130, padding: '13px 15px', lineHeight: 1.5, resize: 'vertical' }}
                  onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => e.target.style.borderColor = 'var(--border-hover)'} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <button type="submit" className="btn btn-red" style={{ height: 50 }}>Mesajı Gönder →</button>
                <span style={{ fontSize: 12.5, color: 'var(--text-3)' }}>Yanıt süresi ~24 saat</span>
              </div>
            </form>
          </div>

          <div>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 23, letterSpacing: '-0.5px', color: 'var(--text)', marginBottom: 18 }}>Sık sorulan sorular</h2>
