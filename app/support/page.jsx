...
const FAQS_EN = [
  { q: 'How often are prices updated?', a: 'We check store prices multiple times a day; and hourly during major sale periods. Your alarm is triggered instantly when a price drops.' },
  { q: 'Is it safe to connect my Steam library?', a: 'Yes. We only read your public game list — we never see your password and cannot make any changes to your account. You can unlink it at any time.' },
  { q: 'How do I set up a price alert?', a: 'Click the "Price Alert" button on a game page, enter your target price; we will send you email and app notifications when the game drops below that price.' },
  { q: 'What does Premium membership offer?', a: 'Unlimited price alerts, historical price charts, country comparisons, and an ad-free experience. It comes with monthly or yearly options, cancel anytime.' },
  { q: 'Can I get a refund?', a: 'You have an unconditional refund right within the first 14 days of your Premium subscription. Just create a request below with the "Premium & Payment" topic.' },
];
...
const TOPICS_EN = [
  { t: 'Account & Membership', d: 'Logging in, resetting password, and profile settings.', icon: <><circle cx="12" cy="8" r="4" /><path d="M5.5 20a6.5 6.5 0 0 1 13 0" /></> },
  { t: 'Prices & Stores', d: 'Price tracking, store comparison, and accuracy.', icon: <><path d="M3 11.5 11.5 3l9.5 9.5-8.5 8.5z" /><circle cx="8" cy="8" r="1.4" /></> },
  ...
  { t: 'Notifications & Alarms', d: 'Managing discount and price alerts.', icon: <><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></> },
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '62px 0 44px', background: 'var(--hero-bg)' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '0 36px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg-card)', border: '1.5px solid var(--border-hover)', borderRadius: 14, height: 58, padding: '0 18px', maxWidth: 540, margin: '0 auto', boxShadow: 'var(--shadow-lg)' }}>
            ...
            <input 
              placeholder={lang === 'tr' ? 'Bir soru veya konu ara… (ör. fiyat alarmı)' : 'Search for a question or topic... (e.g. price alert)'} 
          ...
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 15, padding: 20 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>
                {lang === 'tr' ? 'Şu an çevrimiçi' : 'Currently online'}
              </div>
              <div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 2 }}>
                {lang === 'tr' ? 'Hafta içi 09:00 – 18:00' : 'Weekdays 09:00 – 18:00'}
              </div>
            </div>
          </div>
          <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 15, padding: 20 }}>
        ...
        <div className="support-split" style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 34, alignItems: 'start' }}>
          ...
          <div className="card" style={{ borderRadius: 20, padding: 30, boxShadow: 'var(--shadow)' }}>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 23, letterSpacing: '-0.5px', color: 'var(--text)', marginBottom: 5 }}>
              {lang === 'tr' ? 'Bize yazın' : 'Contact Us'}
            </h2>
            <p style={{ fontSize: 14.5, color: 'var(--text-2)', marginBottom: 24 }}>
              {lang === 'tr' ? 'Formu doldur, ekibimiz en kısa sürede dönsün.' : 'Fill out the form, our team will get back to you as soon as possible.'}
            </p>
